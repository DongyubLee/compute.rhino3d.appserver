...
const definition = 'finalDongyub.gh'
...
resetButton.onclick = zoomCamera
...
function collectResults(responseJson) {
  ...
  loader.parse(arr, function (object) {
    ...
    object.traverse(child => {
      if (child.isMesh) {
        const theCount = child.userData.attributes.geometry.userStringCount
        if (theCount > 0) {
          for ( let i = 0; i < theCount; i ++ ){
            switch (child.userData.attributes.geometry.userStrings[i][0]){
              case 'edge':
    ...
    if (height_slider.valueAsNumber < 25){
      motion = false
    } else {
      motion = true
    }
...
var animate = function () {
  ...
  if ( motion) {
    theMotionX += 0.05
    motionObjX.rotation.z = Math.sin(theMotionX) * 0.05
    theMotionY += 0.1
    motionObjY.position.z = Math.sin(theMotionY) * 0.2
  }
